...
Python의 GIL 기능이 조금 문제인 것으로 확인됨.

---------------------------------------------------------------------------------------------------------------------------

2022년 7월 31일 **일요일**

조금 더 검색해본 결과, GIL이 문제라기 보다는 GIL은 수많은 Thread가 CPU 점유율을 잡아먹는 것을 막기 위한 일종의 방지 장치인 것으로 확인됐다.       
그리고, 해당 작업을 하기 위해선 Thread의 동기화를 사용해야 하는 것 같다.

검색 끝에 완전한 동시 처리능력을 가진 것은 Threading Library가 아닌 Multiprocessing Library이다.      
현재 Python Thread에 관해 이해한 결과, Main Thread와 Sub Thread가 동시에는 작동하나, 만약 Sub Thread가 2개 이상일 때부턴,           
Join() 함수로 첫번째 Thread가 종료될 때까지 기다려야 하고, 종료되면 다음 두번째 Thread가 작동하는 식이다.        
결국 완전한 병렬처리가 아닌, 불완전한 병렬처리로써, 다양한 작업을 하나의 프로세스에서 할 경우가 많은 데, 특히 GIL이 있는 Python에서는 그게 불가능한 셈이다.

그러므로, 이 작업을 위해 MultiProcessing을 좀 더 분석하고 연구해봐야 할 것 같다.

[여기](https://nachwon.github.io/asyncio-futures/) 잘 설명된 곳이 있으니 참고 바람.

--------------------------------------------------------------------------------------------------------------------------

2022년 8월 2일

방향 지시등 구현 코드는 Chapter N 파일이 아닌 AI 자동차에 따로 올려놓도록 하겠다.
구현이 되면 [아직 안함]()를 하도록 하겠다.
